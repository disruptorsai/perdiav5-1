...
function ArticleEditorContent() {
  ...
  // Article editing lock (F-06: Concurrent editing prevention)
  // TEMPORARILY DISABLED - causing navigation freeze issues
  // const {
  //   lockStatus,
  //   hasLock,
  //   isCheckingLock,
  ...
  //   forceTakeLock,
  //   isAcquiringLock
  // } = useArticleEditing(articleId)

  // Stub values while lock feature is disabled
  const lockStatus = null
  const hasLock = false
  const isCheckingLock = false
  const lockError = null
  const acquireLock = async () => true
  const releaseLock = async () => {}
  const forceTakeLock = async () => true
  const isAcquiringLock = false
  ...
  useEffect(() => {
    if (article && !hasLoadedInitialDataRef.current) {
      ...
      // B-06: Store initial values for change detection
  ...
  // B-06: Detect unsaved changes by comparing current state to initial values
  useEffect(() => {
  ...
  // B-06: Reset tracking after successful save
  const resetUnsavedTracking = useCallback(() => {
    initialContentRef.current = content
    initialTitleRef.current = title
    setHasUnsavedChanges(false)
  }, [content, title])

  // B-06: Warn on browser refresh/close with unsaved changes
  useEffect(() => {
  ...
  // F-06: Auto-acquire editing lock when entering editor
  // TEMPORARILY DISABLED - causing navigation freeze issues
  // useEffect(() => {
  //   if (articleId && !isCheckingLock && !hasLock && !lockStatus?.isLocked) {
  //     acquireLock()
  //   }
  // }, [articleId, isCheckingLock, hasLock, lockStatus?.isLocked, acquireLock])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // B-06: Reset unsaved tracking after successful save
      resetUnsavedTracking()
  ...
  const handleAutoFix = async (issues) => {
    try {
      ...
      // Update local content state with the fixed content from the database
      if (result?.content) {
        setContent(result.content)
        // Refetch article to get updated quality score
        refetch()
        toast.success('Quality issues have been automatically fixed')
      } else {
        toast.error('Auto-fix returned empty content')
      }
    } catch (error) {
      console.error('Auto-fix failed:', error)
      toast.error('Auto-fix failed: ' + (error.message || 'Unknown error'))
    }
  }
  ...
  const handleApproveRevision = async () => {
    ...
    try {
      ...
      // B-06/B-07: Reset unsaved tracking since revision is now saved
      initialContentRef.current = pendingRevision.revisedContent
      initialTitleRef.current = title
      setHasUnsavedChanges(false)
  ...
  // B-08: Extract section headings from content for "Regenerate Section" dropdown
  ...
  // B-08: Request Changes handler - quick AI revision for simple changes
  const handleRequestChanges = async () => {
    ...
    try {
      const result = await reviseWithFeedback.mutateAsync({
        content,
        title,
        feedbackItems: [{ comment: requestChangesText.trim(), timestamp: new Date().toISOString() }],
        contentType,
        focusKeyword,
      })

      // Set pending revision for approve/reject workflow
      setPendingRevision({
        previousContent,
        revisedContent: result.content,
        feedbackItems: [{ comment: requestChangesText.trim() }],
        timestamp: new Date().toISOString(),
      })

      setContent(result.content)
      setRequestChangesText('')
      toast.success('Changes ready for review! Approve or reject using the banner above.')
    } catch (error) {
      toast.error('Request changes failed: ' + error.message)
    } finally {
      setIsRequestingChanges(false)
    }
  }

  // B-08: Regenerate Section handler - rewrites a specific section
  const handleRegenerateSection = async () => {
    if (!selectedSectionToRegenerate) {
    ...
    try {
      // Build specific feedback for section regeneration
      const sectionFeedback = regenerateInstructions.trim()
        ? `Completely rewrite the section titled "${selectedSectionToRegenerate}". Instructions: ${regenerateInstructions}`
        : `Completely rewrite and improve the section titled "${selectedSectionToRegenerate}". Make it more engaging, informative, and better written while keeping the same topic.`

      const result = await reviseWithFeedback.mutateAsync({
        content,
        title,
        feedbackItems: [{ comment: sectionFeedback, timestamp: new Date().toISOString() }],
        contentType,
        focusKeyword,
      })
      ...
      setPendingRevision({
        previousContent,
        revisedContent: result.content,
        feedbackItems: [{ comment: `Regenerated section: ${selectedSectionToRegenerate}` }],
        timestamp: new Date().toISOString(),
      })

      setContent(result.content)
      setShowRegenerateSectionDialog(false)
      setSelectedSectionToRegenerate('')
      setRegenerateInstructions('')
      toast.success('Section regenerated! Review and approve or reject the changes.')
    } catch (error) {
      toast.error('Section regeneration failed: ' + error.message)
    } finally {
  ...
  // F-06: Show locked state if another user is editing
  const isLockedByOther = lockStatus?.isLocked && !lockStatus?.isOwnLock
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      {/* F-06: Locked by another user warning banner */}
      {isLockedByOther && (
        <div className="bg-red-50 border-b border-red-200 px-4 py-3 flex-shrink-0">
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-red-900">Article is being edited by another user</p>
                <p className="text-xs text-red-700">
                  {lockStatus?.lockedByEmail} started editing at{' '}
                  {lockStatus?.lockedAt?.toLocaleTimeString()} •
                  Lock expires at {lockStatus?.expiresAt?.toLocaleTimeString()}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <ScrollArea className="flex-1 min-h-0">
                <div className="p-4 space-y-4">
                  ...
                  <TabsContent value="tools" className="mt-0 space-y-4">
                    ...
                    {/* B-08: Request Changes - Quick AI revision for simple changes */}
                    <div className="p-4 bg-purple-50 rounded-lg border border-purple-200 space-y-3">
                      <div className="flex items-center gap-2">
                        <Wand2 className="w-4 h-4 text-purple-600" />
                        <h3 className="font-medium text-sm text-purple-900">Request Changes</h3>
                      </div>
                      <p className="text-xs text-purple-700">
                        Describe what changes you want and AI will make them. Great for quick fixes.
                      </p>
                      <Textarea
                        value={requestChangesText}
                        onChange={(e) => setRequestChangesText(e.target.value)}
                        placeholder="e.g., 'Add a conclusion paragraph', 'Make the tone more professional', 'Add salary statistics for 2026'"
                        rows={2}
                        className="text-xs"
                      ...
                      <Button
                        ...
                        disabled={isRequestingChanges || !requestChangesText.trim() || pendingRevision !== null}
                    ...
                    {/* B-08: Regenerate Section - Rewrite specific sections */}
                    <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-200 space-y-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-medium text-sm text-indigo-900">Regenerate Section</h3>
                      </div>
                      <p className="text-xs text-indigo-700">
                        Completely rewrite a specific section of the article.
                      </p>
                      {sectionHeadings.length > 0 ? (
                        <>
                          <Select
                            value={selectedSectionToRegenerate}
                            onValueChange={setSelectedSectionToRegenerate}
                            disabled={pendingRevision !== null}
                          >
                            <SelectTrigger className="text-xs">
                              <SelectValue placeholder="Select a section to regenerate" />
                            </SelectTrigger>
                            <SelectContent>
                              {sectionHeadings.map((heading, idx) => (
                                <SelectItem key={idx} value={heading}>
                                  {heading.length > 50 ? heading.slice(0, 50) + '...' : heading}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                          {selectedSectionToRegenerate && (
                            <Textarea
                              value={regenerateInstructions}
                              onChange={(e) => setRegenerateInstructions(e.target.value)}
                              placeholder="Optional: Add specific instructions for regenerating this section..."
                              rows={2}
                              className="text-xs"
                              disabled={pendingRevision !== null}
                            />
                          ...
                          <Button
                            ...
                            disabled={isRegeneratingSection || !selectedSectionToRegenerate || pendingRevision !== null}
                        ...
                        <p className="text-xs text-indigo-500 italic">
                          No sections found. Add H2 or H3 headings to your article to enable section regeneration.
                        </p>
                      )}
                    </div>
...
// Note: ToastProvider is already at the App level, no need for nested provider
export default function ArticleEditor() {
  return <ArticleEditorContent />
}
